...
for i = 1:numel(keys)
    ...
    [indexPairs, distances] = matchFeatures(features, curQuarter.features);
    
    % TODO remove
    matchedPoints1 = valid_points(indexPairs(:,1),:);
    matchedPoints2 = curQuarter.points(indexPairs(:,2),:);
    figure; showMatchedFeatures(img, curQuarter.img, matchedPoints1, matchedPoints2);
end
